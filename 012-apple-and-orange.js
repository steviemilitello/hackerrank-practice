...
function countApplesAndOranges(s, t, a, b, apples, oranges) {
    ...
    // filter the position of each fruit
    // filter the position of each fruit by start and end position of house
    // get the length
    let appleResult = apples.map(num => num + a).filter(num => num >= s && num <= t).length
    // do the same for oranges
    let orangeResult = oranges.map(num => num + b).filter(num => num >= s && num <= t).length
